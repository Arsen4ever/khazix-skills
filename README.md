...
随口跟 Agent 说一句"帮我看看存储"或"C 盘满了"，它会扫一遍整机磁盘，在浏览器里打开一份**交互式 HTML 报告**：磁盘总览、占用 Top 5、清理优先级、🟢🟡🔴 三色分级清单。命令一键复制，也可以直接点按钮移到废纸篓。永久删除默认关闭。
...
- 🟢 **绿灯** — 纯缓存、临时文件，删了自动再生。默认一键移到废纸篓
- 🟡 **黄灯** — 含用户数据（离线视频、下载、项目代码）。只给"在访达打开"和"移废纸篓"，让你自己决定，不给直接删
- 🔴 **红灯** — 运行中应用核心数据、系统文件。解释为什么不能动，最多给"打开文件夹"，永远不给删除按钮

**铁律**

全程只读扫描，绝不擅自动手。清理操作必须你在浏览器上确认才执行。本地服务跑在 127.0.0.1 + 随机端口 + token，并校验路径白名单、受保护目录、Host 和 Origin。永久删除必须显式启动，并逐次输入 `DELETE`。
...
本加固分支不使用远程管道安装脚本，请从当前仓库目录安装 `aihot`。
