...
1. **确认PDF转换脚本可用**：本Skill自带 `scripts/md_to_pdf.py`，优先使用 WeasyPrint，不可用时自动回退到本机 Chrome / Edge 的无头打印能力。Python 依赖必须安装在隔离虚拟环境中，不得使用 `--break-system-packages` 污染系统 Python：
   ```bash
   ENV_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/hv-analysis-venv"
   if command -v uv >/dev/null 2>&1; then
     uv venv "$ENV_DIR"
     uv pip install --python "$ENV_DIR/bin/python" markdown
   else
     python3 -m venv "$ENV_DIR"
     "$ENV_DIR/bin/python" -m pip install markdown
   fi
   ```
...
2. **安装依赖**（如未安装）：按“环境准备”创建隔离虚拟环境，禁止写入系统 Python。
3. **运行转换脚本**：
   ```bash
   ENV_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/hv-analysis-venv"
   "$ENV_DIR/bin/python" [skill目录]/scripts/md_to_pdf.py input.md output.pdf --title "研究对象名称" --author "用户指定的作者名"
   ```
4. 脚本会自动生成中间HTML文件（便于调试）和最终PDF

### 脚本内置的排版规范

`md_to_pdf.py` 已内置完整的CSS排版方案，无需手动调整。系统若已正确配置 WeasyPrint 则优先使用；否则自动调用 Chrome、Chromium 或 Edge：
...
PDF文件命名为 `[研究对象名称]_横纵分析报告.pdf`，保存到用户的工作目录中。未收到用户明确署名要求时，使用中性署名“研究报告”，不得冒用方法提出者或其他真实人物的身份。
