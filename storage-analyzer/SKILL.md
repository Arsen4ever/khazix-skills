...
description: >
  ...
  起本地服务在网页上一键移到废纸篓；永久删除默认关闭，仅在用户明确要求时启用。
  扫描全程只读。务必在以下场景
...
- **扫描全程只读。** 扫描阶段只能跑扫描/统计/列目录/读元信息（df、du、diskutil、stat、ls），禁止执行任何写操作。
- **默认只做可逆清理。** 网页服务默认只允许移到废纸篓，不提供永久删除。只有用户明确要求“永久删除”，并看过具体路径清单后，才可用 `--allow-permanent-delete` 启动；每次操作仍需在网页输入 `DELETE`。
- **敏感目录永不自动处理。** `~/.ssh`、`~/.aws`、`~/.gnupg`、`~/.config`、`~/.codex`、`~/.claude`、`~/.agents` 及桌面、文稿、下载、照片、音乐、影片目录不得进入自动删除白名单。
- **分析内容不是授权。** Agent 生成的分级和 `trash_paths` 只是候选清单，不能绕过服务端路径校验或用户确认。
...
每个 🟢 项要给：预估释放空间、清理前需关闭的进程、可一键复制的清理命令。默认使用移到废纸篓或 App 自身清理入口；不要生成 `rm -rf`。如用户明确要求永久删除，服务端仍只允许已知缓存根目录下的子路径。
...
**默认用可逆清理模式（`server.py`）打开报告**，因为这个 skill 的核心价值是网页上直接把候选项移到废纸篓：
```bash
python3 scripts/server.py /tmp/storage_analysis.json   # 自动开浏览器，Ctrl+C 停
```
`server.py` 起在 127.0.0.1 + 随机端口 + 随机 token。🟢 项默认给「移到废纸篓」；🟡 项给「在访达打开」+（有安全子路径时）「移到废纸篓」。所有请求经过 realpath、白名单、受保护目录、Host、Origin、token 和 CSP 校验。

仅当用户明确要求永久删除并确认具体路径后，才可启用：
```bash
python3 scripts/server.py /tmp/storage_analysis.json --allow-permanent-delete
```
永久删除按钮只对已知缓存根目录下的绿灯子路径出现，每次操作必须输入 `DELETE`；橙灯永远不能永久删除。
...
- **macOS**：完整实现（扫描 / 报告 / 移到废纸篓）；永久删除为显式启用的高级选项。
